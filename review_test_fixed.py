...
def test_ai_service_chat():
    ...
    # Check for any of these model names
    model_names = ["deepseek", "llama", "groq"]
    if not any(model_name in metadata["model_used"].lower() for model_name in model_names):
        print(f"❌ Model should be one of {model_names}, got {metadata['model_used']}")
        return False
